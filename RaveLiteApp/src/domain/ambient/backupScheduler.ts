 ...
 * every upcoming chime (plan cadence + Daily Sets + live snoozed / deferred
 * pulses), so chimes survive the app being killed. Rules live in
 * `backupPlanner`; OS calls live in `notifeeScheduler`.
 *
 * Reconciles every minute and whenever the plan or program changes.
 * Runs are serialized: a request during a run queues one follow-up run.
 */
...
import {getActiveHours, readPauseUntil} from './activeHours';
import {
  BACKUP_HORIZON_MS,
  planBackups,
  type BackupCandidate,
} from './backupPlanner';
import {planPulseId} from './planScheduler';
import {pulsePayload} from './pulsePayload';
import {queuedPulses} from './pulseRuntime';
import {setsToday} from './setScheduler';

export const BACKUP_RECONCILE_MS = 60_000;

/**
 * Every chime due within the horizon, in increasing precedence: plan
 * cadence, today's and tomorrow's sets, then pulses the runtime holds (a
 * snoozed or deferred pulse's new time overrides its planned one).
 */
...
export function startBackupScheduler(): void {
  ...
  unsubs = [subscribePlan(onChange), subscribeProgram(onChange)];
}
